...
# 1️⃣ 读取掩膜和原始年龄栅格
mask_raster <- rast("D:/Forest_Fragmentation/人为和自然归因/Edge_raster/Nat_Edge_core_0_1km/Nat_edge_core_0_1km_2020.tif")
age_raster <- rast("D:/Forest_Fragmentation/人为和自然归因/Tree_Age/Age_2020_raw/China_Forest_Age.tif")

# 2️⃣ 将年龄数据重采样到掩膜的分辨率和对齐方式（method="average" 取平均值）
age_1km <- resample(age_raster, mask_raster, method = "average")

# 3️⃣ 可选：用掩膜范围裁剪
age_1km_masked <- mask(age_1km, mask_raster)

writeRaster(
  age_1km_masked,
  filename = "D:/Forest_Fragmentation/人为和自然归因/Tree_Age/Age_2020_1km.tif",
  overwrite = TRUE
)
...
# 1. 读取掩膜栅格（Hum Edge）
mask_path <- "D:/Forest_Fragmentation/人为和自然归因/Edge_raster/Hum_Edge_core_0_1km/Hum_edge_core_0_1km_2020.tif"
mask <- rast(mask_path)

# 2. 读取 Age 栅格（2020 年均值）
Age_path <- "D:/Forest_Fragmentation/人为和自然归因/Tree_Age/Age_2020_raw/Age_2020_1km.tif"
Age <- rast(Age_path)

# 3. 检查 CRS 是否一致，不一致则重投影 Age 到 mask 的 CRS
if (!compareGeom(Age, mask, crs = TRUE, stopOnError = FALSE)) {
  message("CRS 不一致，进行重投影...")
  Age <- project(Age, mask, method = "near")
} else {
  message("CRS 一致，无需重投影")
}

# 4. 重采样对齐（确保栅格分辨率和范围匹配）
Age_aligned <- resample(Age, mask, method = "near")

# 5. 掩膜操作（保留 mask == 1 区域，其他设 NA）
Age_masked <- mask(Age_aligned, mask, maskvalues = 0, updatevalue = NA)

# 6. 输出路径
output_path <- "D:/Forest_Fragmentation/人为和自然归因/Tree_Age/Age_2020_raw/Age_Hum_2020.tif"

# 7. 写出结果，保存为浮点型
writeRaster(Age_masked, output_path, datatype = "FLT4S", NAflag = -9999, overwrite = TRUE)

cat("✅ 掩膜后的 Age 数据已保存至：", output_path, "\n")

# ------------------------------------------------------------------------------9km聚合提取 (Age)
library(terra)

# 1. 读取你的 1km 栅格数据（Age 人为边缘）
r <- rast("D:/Forest_Fragmentation/人为和自然归因/Tree_Age/Age_2020_raw/Age_Nat_2020.tif")
...
# 6. 保存为 CSV 文件
output_csv <- "D:/Forest_Fragmentation/人为和自然归因/Tree_Age/data.summary/Age_Nat_2020_9km.csv"
write.csv(df, output_csv, row.names = FALSE, na = "NA")

cat("✅ 9km 聚合统计已保存至：", output_csv, "\n")


# ------------------------------------------------------------------------------9km聚合提取类型合并，去除NA (Age)
# 1. 读取两个 CSV 文件
hum <- read.csv("D:/Forest_Fragmentation/人为和自然归因/Tree_Age/data.summary/Age_Hum_2020_9km.csv")
nat <- read.csv("D:/Forest_Fragmentation/人为和自然归因/Tree_Age/data.summary/Age_Nat_2020_9km.csv")
...
# 5. 写出为新的 CSV 文件
write.csv(hum_clean, "D:/Forest_Fragmentation/人为和自然归因/Tree_Age/data.summary/Age_Hum_2020_9km_clean.csv", row.names = FALSE)
write.csv(nat_clean, "D:/Forest_Fragmentation/人为和自然归因/Tree_Age/data.summary/Age_Nat_2020_9km_clean.csv", row.names = FALSE)
...
# 1. 读取清理后的两个 CSV 文件
hum_clean <- read.csv("D:/Forest_Fragmentation/人为和自然归因/Tree_Age/data.summary/Age_Hum_2020_9km_clean.csv")
nat_clean <- read.csv("D:/Forest_Fragmentation/人为和自然归因/Tree_Age/data.summary/Age_Nat_2020_9km_clean.csv")
...
p <- ggplot() +
  ...
  coord_cartesian(xlim = c(x_min, x_max), ylim = c(0, 0.12)) +   # 限制横纵坐标范围
...
# 8. 保存图像
ggsave("D:/Forest_Fragmentation/人为和自然归因/Tree_Age/data.summary/∆Age_优化版.png",
...
tif_path <- "D:/Forest_Fragmentation/人为和自然归因/Tree_Age/Age_2020_raw/Age_Hum_2020.tif"
out_dir <- "D:/Forest_Fragmentation/人为和自然归因/Tree_Age/Age_2020_six_region/HumAge_2020_region"
...
for (shp in shp_files) {
  ...
  out_file <- file.path(out_dir, paste0("HumAge_2020_", region_name, ".tif"))
...
# 1. 输入目录（换成 Age 栅格的六大分区）
tif_dir <- "D:/Forest_Fragmentation/人为和自然归因/Tree_Age/Age_2020_six_region/HumAge_2020_region"
out_dir <- "D:/Forest_Fragmentation/人为和自然归因/Tree_Age/Age_2020_six_region/summary_csv"
...
# 1. 文件夹路径（改为 Age 汇总文件夹）
csv_dir <- "D:/Forest_Fragmentation/人为和自然归因/Tree_Age/Age_2020_six_region/summary_csv"
...
# 1. 读取清理后的两个 CSV 文件
hum_clean <- read.csv("D:/Forest_Fragmentation/人为和自然归因/Tree_Age/Age_2020_six_region/summary_csv/clean/Hum_NW_clean.csv")
nat_clean <- read.csv("D:/Forest_Fragmentation/人为和自然归因/Tree_Age/Age_2020_six_region/summary_csv/clean/Nat_NW_clean.csv")
...
# 8. 保存图像
ggsave("D:/Forest_Fragmentation/人为和自然归因/Tree_Age/Age_2020_six_region/summary_csv/clean/∆Age_2020_NW_优化版.png",
       plot = p, width = 10, height = 9, dpi = 600, bg = "white")