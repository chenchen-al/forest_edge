...
# 设置最大 Y 轴范围
max_area <- 6.5  # 设定最大森林边缘面积 (10 × 10¹⁰ m²)
max_percent <- 100  # 设定最大占比（100%）
...
data_long$Edge_Type <- factor(data_long$Edge_Type, 
                              levels = c("Human_edge", "Natural_edge"),
                              labels = c("Anthropogenic Edge", "Natural Edge"))


# 保留柱状图和百分比文本
ggplot(data, aes(x = factor(Year))) +
  geom_bar(data = data_long, aes(y = Edge_Area, fill = Edge_Type),
           stat = "identity", position = position_dodge(width = 0.6),
           width = 0.5, alpha = 0.8) +
  # geom_text(data = data_long, aes(
  #   y = Edge_Area,
  #   label = sprintf("%.2f%%", (Edge_Area / Total_edge) * 100)
  # ),
  # position = position_dodge(width = 0.6),  # 与柱子 dodge 宽度保持一致
  # vjust = -0.8,                             # 更大负值让标签远离柱顶
  # size = 6, color = "black")+                       # 字体加粗提高可读性
  scale_fill_manual(values = c("Anthropogenic Edge" = "gray20", "Natural Edge" = "goldenrod"),guide = "none") +
  labs(
    x = "Year",
    y = "FE Area (× 10¹⁰ m²)"
    # fill = "Type"
  ...
  scale_y_continuous(
    expand = c(0, 0),
    limits = c(0, max_area),
    breaks = seq(0, max_area, by = 1.5),
    labels = scales::number_format(accuracy = 0.1)
  )

# 保存图表
ggsave("D:/Forest_Fragmentation/Picture/FE_area/FE_tape.png", width = 10, height = 12, dpi = 1000, bg = "white")
...
# t 检验
t_test_result <- t.test(human_edge ~ category, data = data, var.equal = TRUE)
...
cat("p 值为：", round(p_value, 4), "\n")
cat("Cohen's d 效应量为：", round(d_value, 3), "（", d_magnitude, "）\n", sep = "")

# 综合解释
cat("\n==== 综合解释 ====\n")
if (p_value < 0.05) {
  cat("两组数据存在显著差异（p < 0.05）。")
  if (d_magnitude == "small") {
    cat(" 但效应量较小，说明实际差异较弱。\n")
  } else if (d_magnitude == "medium") {
    cat(" 效应量为中等，差异具有一定的实际意义。\n")
  } else {
    cat(" 效应量较大，差异具有明确的实际意义。\n")
  }
} else {
  cat("两组数据不存在显著差异（p ≥ 0.05），差异不具统计意义。\n")
}
