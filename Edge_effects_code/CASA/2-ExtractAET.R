...
for(i in seq(1981,2022)){
  AETRasters <- rast(paste0('../NPP/AET/Raw/TerraClimate_aet_',as.character(i),'.nc'))
  for(j in seq(1,12)){
    AETRaster <- AETRasters[[j]]
    outputFileName <- paste0('../NPP/AET/AET_',as.character(i),'_',as.character(j),'.tif')
    writeRaster(AETRaster,outputFileName,overwrite = TRUE)
  }
  print(i)
}
