...
input_folder <- "D:/Forest_Fragmentation/NPP/CASA_China/YZ_MODIS"
output_base <- "D:/Forest_Fragmentation/NPP/CASA_China/YZ_MODIS"
...
# 主目录
base_dir <- "D:/Forest_Fragmentation/NPP/CASA_China/YZ_MODIS"
...
# 打印和保存
print(results)
write.csv(results, file.path(base_dir, "NPP_Total_TgC_MODIS.csv"), row.names = FALSE)
...
# 输出图像
tiff(
  "D:/Forest_Fragmentation/NPP/CASA_China/YZ_MODIS/Regional_NPP_comparison.tif",
  width = 100,      # mm
  height = 90,
  units = "mm",
  res = 900,
...
dat <- data.frame(
  ...
  Opt2023 = c(82.863,171.637,112.483,430.775,349.269,217.516),
  MODIS = c(116.9841,240.303,132.7693,841.188,624.2374,350.7542)
)

# Pearson相关
cor.test(dat$Opt2023, dat$MODIS, method = "pearson")

# 线性回归
fit <- lm(MODIS ~ Opt2023, data = dat)
