...
# 1. 读取清理后的两个 CSV 文件
hum_clean <- read.csv("D:/Forest_Fragmentation/人为和自然归因/PPT_edge2.0/PPT_2020_six_region/summary_csv/clean/Hum_EA_clean.csv")
nat_clean <- read.csv("D:/Forest_Fragmentation/人为和自然归因/PPT_edge2.0/PPT_2020_six_region/summary_csv/clean/Nat_EA_clean.csv")
...
# 8. 保存图像
ggsave("D:/Forest_Fragmentation/人为和自然归因/PPT_edge2.0/PPT_2020_six_region/summary_csv/clean/∆PPT_day_2020_EA_优化版.png",
       plot = p, width = 10, height = 9, dpi = 600, bg = "white")
...
# 2. 读取 PPT 栅格（2020 年均值）
PPT_path <- "D:/Forest_Fragmentation/人为和自然归因/PPT_core/PPT_2020_raw/PPT_2020_annual.tif"
PPT <- rast(PPT_path)
...
# 6. 输出路径
output_path <- "D:/Forest_Fragmentation/人为和自然归因/PPT_core/PPT_2020_raw/PPT_Hum_2020.tif"
...
# 1. 读取你的 1km 栅格数据（PPT 人为边缘）
r <- rast("D:/Forest_Fragmentation/人为和自然归因/PPT_core/PPT_2020_raw/PPT_Nat_2020.tif")
...
# 6. 保存为 CSV 文件
output_csv <- "D:/Forest_Fragmentation/人为和自然归因/PPT_core/data.summary/PPT_Nat_2020_9km.csv"
write.csv(df, output_csv, row.names = FALSE, na = "NA")
...
# 1. 输入路径
shp_dir <- "D:/Forest_Fragmentation/中国标准地图-审图号GS(2020)4619号-shp格式/六大地理分区"
tif_path <- "D:/Forest_Fragmentation/人为和自然归因/PPT_core/PPT_2020_raw/PPT_Hum_2020.tif"
out_dir <- "D:/Forest_Fragmentation/人为和自然归因/PPT_core/PPT_2020_six_region/HumPPT_2020_region"
...
for (shp in shp_files) {
  ...
  # 生成输出文件名（根据shp文件名）
  region_name <- tools::file_path_sans_ext(basename(shp))  # 取文件名去掉扩展名
  out_file <- file.path(out_dir, paste0("HumPPT_2020_", region_name, ".tif"))
...
# 1. 输入目录
tif_dir <- "D:/Forest_Fragmentation/人为和自然归因/PPT_core/PPT_2020_six_region/NatPPT_2020_region"
out_dir <- "D:/Forest_Fragmentation/人为和自然归因/PPT_core/PPT_2020_six_region/summary_csv"
...
# 1. 读取四个 CSV 文件
hum_core <- read.csv("D:/Forest_Fragmentation/人为和自然归因/PPT_edge&core/china清洗/PPT_Hum_core_2020_9km.csv")
hum_edge <- read.csv("D:/Forest_Fragmentation/人为和自然归因/PPT_edge&core/china清洗/PPT_Hum_edge_2020_9km.csv")
nat_core <- read.csv("D:/Forest_Fragmentation/人为和自然归因/PPT_edge&core/china清洗/PPT_Nat_core_2020_9km.csv")
nat_edge <- read.csv("D:/Forest_Fragmentation/人为和自然归因/PPT_edge&core/china清洗/PPT_Nat_edge_2020_9km.csv")
...
# 5. 写出为新的 CSV 文件
write.csv(hum_core_clean, "D:/Forest_Fragmentation/人为和自然归因/PPT_edge&core/china清洗/PPT_Hum_core_2020_9km_clean.csv", row.names = FALSE)
write.csv(hum_edge_clean, "D:/Forest_Fragmentation/人为和自然归因/PPT_edge&core/china清洗/PPT_Hum_edge_2020_9km_clean.csv", row.names = FALSE)
write.csv(nat_core_clean, "D:/Forest_Fragmentation/人为和自然归因/PPT_edge&core/china清洗/PPT_Nat_core_2020_9km_clean.csv", row.names = FALSE)
write.csv(nat_edge_clean, "D:/Forest_Fragmentation/人为和自然归因/PPT_edge&core/china清洗/PPT_Nat_edge_2020_9km_clean.csv", row.names = FALSE)


# ------------------------------------------------------------------------------ 逐列差值（Edge - Core）
# 1. 读取两个清洗后的 CSV 文件
hum_core_clean <- read.csv("D:/Forest_Fragmentation/人为和自然归因/PPT_edge&core/china清洗/PPT_Nat_core_2020_9km_clean.csv")
hum_edge_clean <- read.csv("D:/Forest_Fragmentation/人为和自然归因/PPT_edge&core/china清洗/PPT_Nat_edge_2020_9km_clean.csv")
...
# 5. 输出结果到新文件
write.csv(hum_diff_all, "D:/Forest_Fragmentation/人为和自然归因/PPT_edge&core/china清洗/PPT_NatEdge_minus_Core_2020_9km.csv", row.names = FALSE)
# ------------------------------------------------------------------------------差值作图优化版
library(ggplot2)

# 1. 读取清理后无NA的两个CSV文件
hum_clean <- read.csv("D:/Forest_Fragmentation/人为和自然归因/PPT_edge&core/china清洗/PPT_HumEdge_minus_Core_2020_9km.csv")
nat_clean <- read.csv("D:/Forest_Fragmentation/人为和自然归因/PPT_edge&core/china清洗/PPT_NatEdge_minus_Core_2020_9km.csv")
...
# 8. 保存图像
ggsave("D:/Forest_Fragmentation/人为和自然归因/PPT_edge&core/∆PPT_day_edge&core_优化版.png",
       plot = p, width = 10, height = 9, dpi = 600, bg = "white")
...
# 1. 读取四个文件
hum_core <- read.csv("D:/Forest_Fragmentation/人为和自然归因/PPT_edge&core/china清洗/PPT_Hum_core_2020_9km_clean.csv")
hum_edge <- read.csv("D:/Forest_Fragmentation/人为和自然归因/PPT_edge&core/china清洗/PPT_Hum_edge_2020_9km_clean.csv")
nat_core <- read.csv("D:/Forest_Fragmentation/人为和自然归因/PPT_edge&core/china清洗/PPT_Nat_core_2020_9km_clean.csv")
nat_edge <- read.csv("D:/Forest_Fragmentation/人为和自然归因/PPT_edge&core/china清洗/PPT_Nat_edge_2020_9km_clean.csv")
...
p <- ggplot(df_all, aes(x = group, y = mean, fill = group)) +
  ...
  labs(x = NULL, y = expression(PPT~(mm))) +
...
ggsave("D:/Forest_Fragmentation/人为和自然归因/PPT_edge&core/PPT_China_2020.png",
...
# 1. 文件夹路径
hum_edge_dir <- "D:/Forest_Fragmentation/人为和自然归因/PPT_edge2.0/PPT_2020_six_region/summary_csv"   # 人为边缘
nat_edge_dir <- "D:/Forest_Fragmentation/人为和自然归因/PPT_edge2.0/PPT_2020_six_region/summary_csv"       # 自然边缘
hum_core_dir <- "D:/Forest_Fragmentation/人为和自然归因/PPT_core/PPT_2020_six_region/summary_csv"    # 人为核心
nat_core_dir <- "D:/Forest_Fragmentation/人为和自然归因/PPT_core/PPT_2020_six_region/summary_csv"       # 自然核心
...
# 4. 输出路径
out_dir <- "D:/Forest_Fragmentation/人为和自然归因/PPT_edge&core/region清洗"
if (!dir.exists(out_dir)) dir.create(out_dir)

# 5. 清洗每个区域
for (region in regions) {
  # 构造文件名匹配（注意匹配 summary 和 edge 文件）
  hum_core_file <- hum_core_files[str_detect(hum_core_files, paste0("HumPPT_2020_", region, "_9km_summary\\.csv"))]
  hum_edge_file <- hum_edge_files[str_detect(hum_edge_files, paste0("HumPPT_2020_", region, "_9km_summary\\.csv"))]
  nat_core_file <- nat_core_files[str_detect(nat_core_files, paste0("NatPPT_2020_", region, "_9km_summary\\.csv"))]
  nat_edge_file <- nat_edge_files[str_detect(nat_edge_files, paste0("NatPPT_2020_", region, "_9km_summary\\.csv"))]
...
# 设置输入输出路径
input_dir  <- "D:/Forest_Fragmentation/人为和自然归因/PPT_edge&core/region清洗"
output_dir <- "D:/Forest_Fragmentation/人为和自然归因/PPT_edge&core/region清洗"
...
for (region in regions) {
  for (type in types) {
    ...
    # 写入结果
    output_file <- file.path(output_dir, paste0(type, "Edge_minus_Core_PPT_", region, "_9km.csv"))
...
# 1. 读取清理后无NA的两个CSV文件
hum_clean <- read.csv("D:/Forest_Fragmentation/人为和自然归因/PPT_edge&core/region清洗/HumEdge_minus_Core_PPT_SW_9km.csv")
nat_clean <- read.csv("D:/Forest_Fragmentation/人为和自然归因/PPT_edge&core/region清洗/NatEdge_minus_Core_PPT_SW_9km.csv")
...
p <- ggplot() +
  ...
  # 黑色虚线 x=0
  geom_vline(xintercept = 0, color = "black", linetype = "dashed", linewidth = 1.5) +
   coord_cartesian(xlim = c(x_min, x_max), ylim = c(0, 0.07)) +   # 限制横纵坐标范围
  # coord_cartesian(xlim = c(x_min, x_max), ylim = c(0, max(df_dens$y) * 1.05)) +
  # labs(x = "∆PPT(℃)", y = "Density") +
  theme_minimal(base_size = 45) +
  theme(
...
# 8. 保存图像
ggsave("D:/Forest_Fragmentation/人为和自然归因/PPT_edge&core/∆PPT_day_edge&core_SW_优化版.png",
       plot = p, width = 10, height = 9, dpi = 600, bg = "white")
...
# 1. 读取四个文件
hum_core <- read.csv("D:/Forest_Fragmentation/人为和自然归因/PPT_edge&core/region清洗/HumCore_NE_clean.csv")
hum_edge <- read.csv("D:/Forest_Fragmentation/人为和自然归因/PPT_edge&core/region清洗/HumEdge_NE_clean.csv")
nat_core <- read.csv("D:/Forest_Fragmentation/人为和自然归因/PPT_edge&core/region清洗/NatCore_NE_clean.csv")
nat_edge <- read.csv("D:/Forest_Fragmentation/人为和自然归因/PPT_edge&core/region清洗/NatEdge_NE_clean.csv")
...
p <- ggplot(df_all, aes(x = group, y = mean, fill = group)) +
  ...
  labs(x = NULL, y = expression(PPT~(mm))) +
  coord_cartesian(ylim = c(0, 3800)) +   # 限制纵坐标最大值为40
  scale_y_continuous(breaks = seq(0, 3800, by = 1000)) +  # 每隔20一个刻度
  theme_minimal(base_size = 45) +
  theme(
    panel.grid = element_blank(),
    panel.border = element_rect(color = "black", fill = NA, linewidth = 2.5), # 外框加粗
...
ggsave("D:/Forest_Fragmentation/人为和自然归因/PPT_edge&core/PPT_NE_2020.png",
       plot = p, width = 10, height = 10, dpi = 600 ,bg = "white")
